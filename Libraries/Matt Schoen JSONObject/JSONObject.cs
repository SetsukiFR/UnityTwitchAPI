...
#endif
								if(!str.Contains(".")) {
									i = System.Convert.ToInt64(str, System.Globalization.CultureInfo.InvariantCulture);
									useInt = true;
								}
								type = Type.NUMBER;
	...
	public static implicit operator WWWForm(JSONObject obj) {
		...
		for(int i = 0; i < obj.list.Count; i++) {
			...
			if(obj.type == Type.OBJECT)
				key = obj.keys[i];
			string val = obj.list[i].ToString(System.Globalization.CultureInfo.InvariantCulture);
